  ...
  return (
    <div className="table-container">
      ...
      <table id="toprent-table">
        <tbody>
          {data.map((item, index) => (
            <Paper elevation={10} style={{ backgroundColor: 'black', borderRadius: '10px', marginTop: '12px', marginBottom: '12px' }}>
            <tr key={item.id} onClick={() => handleTitleClick(item)} style={{ cursor: 'pointer' }}>
              <td>{index+1}. {titleFix(item.title)} <img src={reel} id="reel-pic" alt="" /></td>
            </tr>
            </Paper>
          ))}
        </tbody>
      ...
      <Dialog open={open} onClose={handleClose}>
        <DialogTitle style={{backgroundColor: 'gold', paddingBottom: '10px'}}>Movie Details</DialogTitle>
        <DialogContent className="toprent-dialog">
          {selectedMovie && (
            <div className="toprent-dialog">
              <p><strong>Title:</strong> {titleFix(selectedMovie.title)}</p>
              <p><strong>Times Rented:</strong> {selectedMovie.rented} times</p>
